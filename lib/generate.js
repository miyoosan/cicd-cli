...
async function genCDFiles(gitlabUrl, namespace, imageName, containerPort, redis, dingtalkCICDWebhook, phone, _servicePort, cluster) {
  const [_imageRepository, imageHash] = imageName.split(':');
  const imageRepository = cluster.indexOf('gcp') > -1 ? imageRepository.replace(/-vpc/, '') : _imageRepository;
